...
- **AI-Driven Insights**: Uses OpenRouter AI (Qwen) to provide architectural observations and recommendations.
...
- OpenRouter API Key (for insights and cost estimation)
...
- **AI Model**: The app is configured to use OpenRouter's `qwen/qwen-2.5-72b-instruct:free` model for cost estimation and insights.
- **Puppeteer Headless**: Some websites use heavy anti-bot protections; `puppeteer-extra-plugin-stealth` is used to mitigate this.
- **Caching**: Results are cached in Redis for 24 hours to ensure speed and cost-efficiency (falls back to memory if Redis is down).
